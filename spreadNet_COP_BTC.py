...
def balancing_Ask_Bid():
    ...
    if abs(pastAsks-pastBids)<=minVolumeTrade:
        ...
        if (round_decimals_down(pastBids-pastAsks,5)>volume):
            ...
            load_dotenv("spreadNet_"+MONEY.upper()+"_"+CRYPT.upper()+".env")
...
sleepError=int(os.getenv('sleepError'))
sleepApis=int(os.getenv('sleepApis'))
sleepErrorApis=int(os.getenv('sleepErrorApis'))

URL="https://www.buda.com/api/v2/markets/"+CRYPT.lower()+"-"+MONEY.lower()+"/order_book.json"

owners_warning=os.getenv('owners_warning')

priceDistance=float(os.getenv('priceDistance'))
minVolumeTrade=float(os.getenv('minVolumeTrade'))
marketDecimals=int(os.getenv('marketDecimals'))
tradeProportion=float(os.getenv('tradeProportion'))
utilityMarginThreshold=float(os.getenv('utilityMarginThreshold'))
maxTradingVolumeProportion=float(os.getenv('maxTradingVolumeProportion'))
